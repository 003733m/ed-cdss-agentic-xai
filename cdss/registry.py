...
def load_task_registry(
    ...
    """
    Expects artifacts_dir contains:
      - htn_model.joblib, htn_shap_explainer.pkl, htn_artifact.json
      - hf_model.joblib,  hf_shap_explainer.pkl,  hf_artifact.json
    """
    # --- HTN ---
    htn_meta = _load_json(os.path.join(artifacts_dir, "htn_artifact.json"))
    htn_model = joblib.load(os.path.join(artifacts_dir, "htn_model.joblib"))

    htn_explainer_path = os.path.join(artifacts_dir, "htn_shap_explainer.pkl")
    htn_explainer = joblib.load(htn_explainer_path) if os.path.exists(htn_explainer_path) else None
    ...
    # --- HF ---
    hf_meta = _load_json(os.path.join(artifacts_dir, "hf_artifact.json"))
    hf_model = joblib.load(os.path.join(artifacts_dir, "hf_model.joblib"))

    hf_explainer_path = os.path.join(artifacts_dir, "hf_shap_explainer.pkl")
    hf_explainer = joblib.load(hf_explainer_path) if os.path.exists(hf_explainer_path) else None
    ...
    # Task-specific core/opt splits (same spirit as your v21.1 cell)
    ...
    hf_opt_cols = [c for c in cols_from_groups(df, FEATURE_GROUPS, [
    ...
    registry: Dict[str, TaskConfig] = {
        ...
        TASK_HF: TaskConfig(
            ...
            leakage_note="PretestPolicy: MCQ/CDQ/BPX/BPQ/DIQ/RX hariç (geçmiş/tanı sızıntısız).",
